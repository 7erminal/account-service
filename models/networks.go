...
type Networks struct {
	...
	NetworkCode        string     `orm:"column(network_code);size(80);null"`
	Id                 int        `orm:"column(network_id);auto"`
	NetworkReferenceId string     `orm:"column(network_reference_id);size(250);null"`
	OperatorId         *Operators `orm:"column(operator_id);rel(fk)"`
}

func (t *Networks) TableName() string {
	return "networks"
}
...
// Id doesn't exist
func GetNetworksById(id int) (v *Networks, err error) {
	o := orm.NewOrm()
	v = &Networks{Id: id}
...
func DeleteNetworks(id int) (err error) {
	o := orm.NewOrm()
	v := Networks{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Networks{Id: id}); err == nil {
