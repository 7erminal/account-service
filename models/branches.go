...
type Branches struct {
	...
	Branch        string    `orm:"column(branch);size(80)"`
	Id            int       `orm:"column(branch_id);auto"`
	BranchManager int       `orm:"column(branch_manager);null"`
	CountryId     int       `orm:"column(country_id);null"`
	...
	Location      string    `orm:"column(location);size(200);null"`
	ModifiedBy    int       `orm:"column(modified_by);null"`
	PhoneNumber   string    `orm:"column(phone_number);size(20);null"`
}

func (t *Branches) TableName() string {
	return "branches"
}
...
// Id doesn't exist
func GetBranchesById(id int) (v *Branches, err error) {
	o := orm.NewOrm()
	v = &Branches{Id: id}
...
func DeleteBranches(id int) (err error) {
	o := orm.NewOrm()
	v := Branches{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Branches{Id: id}); err == nil {
