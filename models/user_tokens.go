...
type UserTokens struct {
	...
	Nonce        string    `orm:"column(nonce);size(255);null"`
	Token        string    `orm:"column(token);size(255)"`
	Id           int       `orm:"column(user_token_id);auto"`
}

func (t *UserTokens) TableName() string {
	return "user_tokens"
}
...
// Id doesn't exist
func GetUserTokensById(id int) (v *UserTokens, err error) {
	o := orm.NewOrm()
	v = &UserTokens{Id: id}
...
func DeleteUserTokens(id int) (err error) {
	o := orm.NewOrm()
	v := UserTokens{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&UserTokens{Id: id}); err == nil {
